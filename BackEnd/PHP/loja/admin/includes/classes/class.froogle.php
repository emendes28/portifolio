...
class ISC_ADMIN_FROOGLE extends ISC_ADMIN_AJAXEXPORTER
{
	...
	protected function WriteHeader()
	{
				...
				<feed xmlns="http://www.w3.org/2005/Atom" xmlns:g="http://base.google.com/ns/1.0">
					...
					<link rel="self" href="'.getConfig('ShopPathNormal').'"/>
	...
	protected function GetResult($generateFull = false, $start = 0)
	{
			...
			FROM
				...
				LEFT JOIN [|PREFIX|]product_images pi ON (pi.imageprodid = p.productid AND pi.imagesort=0)
			WHERE
				p.prodvisible=1
			";
		if (!$generateFull) {
	...
	public function WriteRow($row)
	{
		...
		$price = CalcRealPrice($row['prodprice'], $row['prodretailprice'], $row['prodsaleprice'], $row['prodistaxable']);
