...
class ISC_ADMIN_STATISTICS_ORDERS extends ISC_ADMIN_STATISTICS
{
	...
	*/
	public function OrderStats()
	{
		...
		$GLOBALS['SalesTaxSummary'] = GetLang('SalesTaxSummary', array('numOrders' => $totalsRow['numorders'], 'taxTotal' => FormatPrice((float)$totalsRow['taxamount'])));
	...
	**/
	public function OrderStatsByAbandonGrid()
	{
		...
		if(isset($_GET['From']) && isset($_GET['To'])) {
			...
			$query = "
				SELECT
					COUNT(*) AS num,
					SUM(ordtotalamount) as totalamount
				FROM
					[|PREFIX|]orders
				WHERE
					ordstatus = 0 AND
			...
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("stats.orders.byabandongrid");
			$GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate();
		}
	}
	...
	public function TaxStatsByDateGrid()
	{
		...
		while (true) {
			if (!isset($dataRows[$currentStamp])) {
				...
				foreach ($dataRows[$currentStamp] as $row) {
					$outputRows[] = array(
						'period'	=> $currentStamp,
						'taxtype'	=> $row['ordtaxname'],
						'taxrate'	=> $row['ordtaxrate'],
		...
		$outputRows = array_msort($outputRows, array($sortField => $arraySort));
		...
		$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("stats.orders.salestax");
		$GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate();
	}
}