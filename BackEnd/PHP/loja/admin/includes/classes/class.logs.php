...
class ISC_ADMIN_LOGS
{
	...
	private function AdministratorLogGrid()
	{
		...
		if ($numEntries > 0) {
			...
			while ($row = $GLOBALS["ISC_CLASS_DB"]->Fetch($result)) {
				if(!$row['username']) {
					...
					$GLOBALS['Username'] = sprintf("<a href='index.php?action=editUser&amp;userId=%d'>%s</a>", $row['loguserid'], $row['username']);
				}
				...
				$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("logs.administrator.row");
				$GLOBALS['LogGrid'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate(true);
			}
		...
		$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("logs.administrator.grid");
	...
	private function SystemLogGrid()
	{
		...
		if(isset($_REQUEST['logsummary']) && $_REQUEST['logsummary'] != 0) {
		...
		$logTypes = array(
			...
			'ssnx' => GetLang('LogSearchSendStudio'),
		...
		if ($numEntries > 0) {
			...
			while ($row = $GLOBALS["ISC_CLASS_DB"]->Fetch($result)) {
				...
				$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("logs.system.row");
				$GLOBALS['LogGrid'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate(true);
			}
		}
		else {
		...
		$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("logs.system.grid");
		return $GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate(true);
	...
	private function ShowSystemLog($MsgDesc = "", $MsgStatus = "")
	{
		...
		$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("logs.system");
