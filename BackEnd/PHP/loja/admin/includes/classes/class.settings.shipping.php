<?php
class ISC_ADMIN_SETTINGS_SHIPPING
{
	/**
	 * The constructor.
	 */
	...
	private function AddShippingZoneMethod()
	{
		...
		$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate('shippingzone.method.form');
	...
	private function EditShippingZone()
	{
		...
		while($method = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
			...
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate('shippingzone.form.method');
			$GLOBALS['ShippingZoneMethods'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate(true);
		}
		...
		$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintHeader();
		$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate('shippingzone.form');
		$GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate();
		$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintFooter();
	}
	...
	public function ManageShippingZonesGrid(&$numZones)
	{
		...
		while($zone = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
			...
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("shippingzones.manage.row");
			$gridRow = $GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate(true);
			if($zone['zoneid'] == 1) {
		...
		$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("shippingzones.manage.grid");
	...
	private function ManageShippingSettings()
	{
		...
		$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintHeader();
		$GLOBALS["ISC_CLASS_TEMPLATE"]->SetTemplate('settings.shipping.manage');
		$GLOBALS["ISC_CLASS_TEMPLATE"]->ParseTemplate();
		$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintFooter();
	}
}