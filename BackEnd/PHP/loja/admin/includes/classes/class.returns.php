...
class ISC_ADMIN_RETURNS
{
	...
	private function ManageReturnsGrid(&$numReturns)
	{
		...
		while ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($returnResult)) {
			...
			if($row['ordtotalincludestax'] == 0 && $row['ordtaxrate'] > 0) {
				$taxCharged = number_format(($returnAmount/100)*$row['ordtaxrate'], GetConfig('DecimalPlaces'), '.', '');
				$returnAmount += $taxCharged;
			}
			...
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("returns.manage.row");
			$GLOBALS['ReturnGrid'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate(true);
		}
		$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("returns.manage.grid");
	...
	private function IssueReturnCredit()
	{
		...
		if($order['ordtotalincludestax'] == 0 && $order['ordtaxrate'] > 0) {
			$taxCharged = number_format(($additionalCredit/100)*$order['ordtaxrate'], GetConfig('DecimalPlaces'), '.', '');
			$additionalCredit += $taxCharged;
		}
	...
	public function UpdateReturnStatus(&$return, $status, $crediting = false)
	{
		...
		if($status == 5 && $return['retstatus'] != 5) {
			...
			$updatedOrder = array(
				"ordsubtotal" => $orderSubTotal,
				"ordtotalamount" => $orderTotalAmount
			);
			...
			// If tax is included within the order total, then we need to do a little more work
			else if($order['ordtotalincludestax']) {
	...
	public function ManageReturns($MsgDesc = "", $MsgStatus = "")
	{
		...
		$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("returns.manage");
	...
	private function SearchReturns()
	{
		...
		$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("returns.search");
		$GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate();
	}
	...
	*/
	...
	{
		$GLOBALS['ReturnStatusOptions'] = $this->GetReturnStatusOptions();
		$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("returns.view");
		$GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate();
	}
