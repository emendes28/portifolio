	...
	*/
	private function manageOptimizer()
	{
		...
		foreach ($Tests as $Test) {
			...
			$GLOBALS['OptimizerRow'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet('OptimizerRow');

		}

		$GLOBALS["ISC_CLASS_TEMPLATE"]->SetTemplate("optimizer.manage");
	...
	*/
	public function showPerItemConfigForm($type, $itemId, $testPageUrl='', $testType='multivariate')
	{
		...
		if($itemId) {
			...
			$GLOBALS['ValidateConversionPageUrl'] = $GLOBALS['ShopPathNormal'].'/validarotimizacao.php?id='.$type.'_'.$itemId;
		}
		...
		return $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet('OptimizerConfigForm');
	}
