...
class ISC_ADMIN_LAYOUT
{
	...
	public function LoadChooseTemplateTab()
	{
		...
		foreach($templateList as $template) {
			...
			$GLOBALS['TemplateListMap'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate(true);
		}
	}
	...
	public function GetEmailTemplateRows($directory='', $parentRow='')
	{
		...
		foreach($files as $file => $type) {
			...
			if(is_dir($filePath)) {
				...
				$output .= $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet('EmailTemplateDirectory');
			}
			else {
				$GLOBALS['FileSize'] = NiceSize(filesize($filePath));
