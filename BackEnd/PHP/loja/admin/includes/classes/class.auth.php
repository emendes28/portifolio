<?php
	...
	class ISC_ADMIN_AUTH
	{
		...
		public function ProcessLogin()
		{
			$loginName='';
			$loginPass='';
			if((!isset($_POST['usuario']) || !isset($_POST['senha'])) && !isset($_COOKIE['RememberToken'])) {
			...
			if(isset($_POST['usuario'])) {
				$loginName = @$_POST['usuario'];
				$loginPass = @$_POST['senha'];
			...
			if($userRow = $GLOBALS["ISC_CLASS_DB"]->Fetch($userResult)) {
				...
				if($userRow['userimportpass'] != '' && $userRow['userpass'] != md5($loginPass)) {
					if(ValidateImportPassword($loginPass, $userRow['userimportpass'])) {
						// Valid login from an import password. We now store the Loja Virtual V2010 version of the password
						$updatedUser = array(
		...
		public function DoLogin($BadLogin=false, $PassUpdated=false)
		{
			...
			if($BadLogin) {
				...
				if(isset($_POST['usuario'])) {
					$GLOBALS['Username'] = isc_html_escape($_POST['usuario']);
				}
