<?php
	...
	/**
	*	This class handles everything to do with E-Marketer V6 integration into Loja Virtual V2010
	*
	*	@author Mitchell Harper
	*	@date November 1st 2007
	*	@copyright www.google.com.br.
	*/
	class ISC_ADMIN_SENDSTUDIO
	{

		/**
		*	Get a list of all mailing lists available for the E-Marketer V6 user and return them as <option> tags
		*/
		public function GetAvailableMailingListsAsoptions($Selected="", &$NumLists = 0)
		{
			// Get a list of available mailing lists from E-Marketer V6
			$output = "";
			$list_xml = "<xmlrequest>
		...
		/**
		*	Get a list of all mailing lists available for the E-Marketer V6 user
		*/
		...
		public function AddSubscriberToNewsletter($FirstName, $Email)
		{
			...
			// Add the subscriber
				...
				$add_xml = "<xmlrequest>
							...
							<details>
								<format>". GetConfig('MailNewsletterFormat') ."</format>
				...
				if(!$this->SubscriberExists(GetConfig('MailOrderList'), $order['ordbillemail'])) {
					...
					$add_xml = "<xmlrequest>
								...
								<details>
									...
									<format>".GetConfig('MailOrderFormat')."</format>
		...
		public function AddSubscriberToList($Email, $ListId)
		{
			...
			if(!$this->SubscriberExists($ListId, $Email)) {
				...
				$add_xml = "<xmlrequest>
							...
							<details>
								<confirmed>yes</confirmed>
								<format>".GetConfig('MailProductUpdatesFormat')."</format>
								<emailaddress>" . $Email . "</emailaddress>
