<?php
	class ISC_ADMIN_SETTINGS
	{
		...
		private function SaveUpdatedSettings()
		{
			...
			// Image dimensions have been changed - we need to go and remove existing thumbnails
			if(!empty($changedDimensions)) {
				$imageIterator = new ISC_PRODUCT_IMAGE_ITERATOR;
				foreach($imageIterator as $image) {
					foreach($changedDimensions as $dimension) {
						$image->removeResizedFile($dimension);
					}
				}
			}
		...
		private function ManageSettings($messages=array())
		{
			...
			*/
			...
			// Hide the special offers tickbox if E-Marketer V6 isn't integrated
		...
		private function SaveUpdatedMailSettings()
		{
			...
			if (isset($_POST['MailXMLPath']) && isset($_POST['MailXMLToken']) && isset($_POST['MailUsername'])) {
				...
				if (isset($_POST['UseMailAPIForNewsletters'])) {
					...
					$GLOBALS['ISC_NEW_CFG']['MailNewsletterFormat'] = $_POST['MailNewsletterFormat'];
					...
					$GLOBALS['ISC_NEW_CFG']['MailNewsletterFormat'] = 't';
				...
				if (isset($_POST['UseMailAPIForOrders'])) {
					...
					$GLOBALS['ISC_NEW_CFG']['MailOrderFormat'] = $_POST['MailOrderFormat'];
					...
					$GLOBALS['ISC_NEW_CFG']['MailOrderFormat'] = 't';
				...
				if(isset($_POST['UseMailAPIForUpdates'])) {
					...
					$GLOBALS['ISC_NEW_CFG']['MailProductUpdatesFormat'] = $_POST['MailProductUpdatesFormat'];
				}
				else {
					$GLOBALS['ISC_NEW_CFG']['UseMailAPIForUpdates'] = 0;
					$GLOBALS['ISC_NEW_CFG']['MailProductUpdatesListType'] = "";
					$GLOBALS['ISC_NEW_CFG']['MailProductUpdatesFormat'] = 't';
				}
		...
		*/
		...
		/**
		* Allow the user to enter their affiliate tracking code which will be placed on the formadepagamento.php page
		*
		* @return Void
		*/
