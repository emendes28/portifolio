...
class ISC_BATCH_IMPORTER_PRODUCT_VARIATIONS extends ISC_BATCH_IMPORTER_BASE
{
	...
	protected function _ImportRecord($record)
	{
		...
		if (empty($prodIdentField)) {
			$this->ImportSession['Results']['Failures'][] = GetLang('NoIdentField', array('identField' => $identFieldName));
			return;
		}
		...
		if (($prod = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) === false) {
			...
			$this->ImportSession['Results']['Failures'][] = GetLang('AssociatedProductNotFound', array('identField' => $identFieldName, 'identValue' => $record[$prodIdentField]));
		...
		if(!empty($record['prodvarimage'])) {
			...
			if (preg_match('#^(?P<scheme>[a-zA-Z0-9\.]+)://#i', $imageFile, $matches)) {
				...
				try {
					$variationImage = ISC_PRODUCT_IMAGE::importImage($importImageFilePath, basename($importImageFilePath), false, false, true, false);
			...
			if ($variationImage !== false) {
				$updateFields['vcimage'] = $variationImage->getSourceFilePath();
				$updateFields['vcimagezoom'] = $variationImage->getResizedFilePath(ISC_PRODUCT_IMAGE_SIZE_ZOOM, true, false);
				$updateFields['vcimagestd'] = $variationImage->getResizedFilePath(ISC_PRODUCT_IMAGE_SIZE_STANDARD, true, false);
				$updateFields['vcimagethumb'] = $variationImage->getResizedFilePath(ISC_PRODUCT_IMAGE_SIZE_THUMBNAIL, true, false);
			}
		}
		...
		foreach ($variationFields as $field) {
			...
			if (count($varField) != 2) {
				$this->ImportSession['Results']['Failures'][] = GetLang('CantExtractData', array('dataField' => $field));
		...
		if (empty($variationData)) {
			// generate a failure
			$this->ImportSession['Results']['Failures'][] = GetLang('NoVariationData', array('rowNum' => $this->ImportSession['DoneCount'] + 1));
		...
		if ($this->ImportSession['UpdateExisting'] && $variationID > 0 && !isset($this->ImportSession['NewVariations'][$productID])){
			...
			if ($comboID = $GLOBALS['ISC_CLASS_DB']->FetchOne($result, 'combinationid')) {
				...
				$this->ImportSession['Results']['Updates'][] = $prod['prodname'];
			}
			else {
...
function GetVariationCombinations($productID, $excludeOption = '')
{
	...
	return array_cartesian_product($optionArray, true);
}