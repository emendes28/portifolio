...
class ISC_ADMIN_UPGRADE_5500 extends ISC_ADMIN_UPGRADE_BASE
{
	...
	public function pre_upgrade_checks()
	{
		...
		$error = 'Loja Virtual V2010 requires all database tables and columns to have their collation set to case-insensitive. ';
	...
	*/
	...
	private function _finaliseProductImageRemovals ()
	{
		...
		if (count($this->_deleteImageList)) {
	...
	*/
	...
	private function _finaliseProductThumbnails ()
	{
		...
		if (count($this->_imageThumbnails)) {
	...
	private function rebuildSuggestedData($type)
	{
		...
		$result = $GLOBALS["ISC_CLASS_DB"]->Query($query);
		while ($row = $GLOBALS["ISC_CLASS_DB"]->Fetch($result)) {
			manageSuggestedWordDatabase($type, $row[$idColumn], $row[$nameColumn]);
		}
