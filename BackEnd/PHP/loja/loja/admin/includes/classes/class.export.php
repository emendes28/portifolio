	...
	*/
	...
	private function RunExport()
	{
		try {
			...
			elseif (isset($_REQUEST['searchId'])) {
				// get the where statement for this search
				$ret = $this->filetype->GetWhereFromSearch($_REQUEST['searchId']);
				$where = $ret['where'];
			}
			...
			$details = $this->filetype->GetTypeDetails();
			if ($_REQUEST['format'] == "CSV" && $details['name'] == "customers" && $method->settings['AltCustomers']) {
			...
			// log the export
			//$GLOBALS['ISC_CLASS_LOG']->LogAdminAction($this->type_title, $templateName, $method_details['name']);

			// send the file to the user
			//DownloadFile($file, $this->type . "-" . isc_date("Y-m-d") . "." . $method_details['extension'], true);
