...
class ISC_ADMIN_CART_API extends ISC_CART_API
{
	...
	public function LoadInOrderItems($orderId)
	{
		...
		while($product = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
			...
			// if this product has a variation it means variations are required
			$optionsreq = 0;
			...
			$cartItem['data'] = array(
				...
				'prodcalculatedprice' => $product['ordprodoriginalcost'],
				...
				'proddepth' => 0,
				'prodweight' => 0,
				'prodoptionsrequired' => $optionsreq,
				'categoryids' => ''
			);
			...
			$query = "
				...
				FROM
					[|PREFIX|]products p
				WHERE
					p.productid = " . $cartItem['product_id'];
			$rescheck = $GLOBALS['ISC_CLASS_DB']->Query($query);
			if ($checkProduct = $GLOBALS['ISC_CLASS_DB']->Fetch($rescheck)) {
				$loadFields = array('prodcalculatedprice', 'prodinvtrack', 'prodcurrentinv', 'prodfreeshipping', 'prodwidth', 'prodheight', 'proddepth', 'prodweight', 'prodoptionsrequired', 'categoryids');
	...
	public function AddItem($productId, $quantity=1, $variationDetails=null, $configurableOptions=array(), $cartItemId=null, $options=array())
	{
		...
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		$options['data'] = $GLOBALS['ISC_CLASS_DB']->Fetch($result);
		...
		if ($variationDetails) {
			...
			if($variationInfo['vcthumb']) {
				$options['data']['imagefile'] = $variationInfo['vcthumb'];
			}

			if($variationInfo['vcweight'] != 0) {
				$options['data']['prodweight'] = CalcProductVariationWeight($options['data']['prodweight'], $variationInfo['vcweightdiff'], $variationInfo['vcweight']);
			}

			// Store the # of this combination we have in stock
			if($options['data']['prodinvtrack'] == 2) {
				$options['data']['prodcurrentinv'] = $variationInfo['vcstock'];
			}
		}
	...
	{
		if (($itemId = parent::AddVirtualItem($productDetails, $itemId)) === false) {
			return false;
		}
