...
class ISC_ADMIN_STATISTICS_OVERVIEW extends ISC_ADMIN_STATISTICS
{
	...
	*/
	public function Overview()
	{
		...
		$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("stats.overview");
		...
		if(isset($_GET['from']) && is_numeric($_GET['from']) && isset($_GET['to']) && is_numeric($_GET['to'])) {
			...
			if($num_days <= 1) {
				...
				$query = "
					SELECT orddate
					FROM [|PREFIX|]orders
					WHERE orddate >= '".$from."' AND orddate <= '".$to."' AND ordstatus IN (".implode(',', GetPaidOrderStatusArray()).")
					".$vendorSql."
				";
				$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
	...
	public function _Top20ProductsGrid($FromStamp, $ToStamp, &$NumResults)
	{
		...
		$query = "
			SELECT
				ordprodname,
				SUM(ordprodqty) AS numsold,
				SUM(ordprodcost * ordprodqty) as total,
				productid
			FROM
				[|PREFIX|]order_products
				INNER JOIN [|PREFIX|]orders ON orderorderid = orderid
