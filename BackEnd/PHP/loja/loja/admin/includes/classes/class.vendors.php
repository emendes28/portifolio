...
class ISC_ADMIN_VENDORS
{
	...
	private function ManageVendorsGrid(&$numVendors)
	{
		...
		while($vendor = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
			...
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("vendors.manage.row");
			$GLOBALS['VendorsGrid'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate(true);
		}
		$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("vendors.manage.grid");
	...
	public function ManageVendors()
	{
		...
		$GLOBALS["ISC_CLASS_TEMPLATE"]->SetTemplate('vendors.manage');
	...
	private function AddVendor()
	{
		...
		$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate('vendor.form');
		$GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate();
	}
