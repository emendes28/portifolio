...
class ISC_ADMIN_PRODUCT_IMAGE {
	...
	*/
	public function remoteUseSourceImages (ISC_ADMIN_REMOTE $remote)
	{
		...
		if (!count($errors) && count($sourceImages)) {
	...
	*/
	public function importImagesFromUrls ($productId, $urls, &$images, &$errors, $hash = false)
	{
		foreach ($urls as $originalUrl) {
			...
			try {
				$image = ISC_PRODUCT_IMAGE::importImage($temporaryPath, $originalFilename, $productId, $hash);
			} catch (ISC_PRODUCT_IMAGE_IMPORT_INVALIDIMAGEFILE_EXCEPTION $exception) {
