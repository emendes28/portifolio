	...
	public function ProductStatsByInventoryGrid()
	{
		...
		if ($total_products > 0) {
			...
			if ($showVariations) {
				//	the structure of this query is different when variations are displayed
				$query = "
					SELECT productid, prodcode, prodname, prodnumviews, prodinvtrack, combinationid, voname, vcstock, vcsku,
						CASE prodinvtrack
							WHEN '0' THEN
								-1
							WHEN '1' THEN
						...
						CASE prodinvtrack
							WHEN '2' THEN
								(SELECT GROUP_CONCAT(CONCAT(voname, ': ', vovalue) ORDER BY vooptionsort ASC SEPARATOR ', ') FROM [|PREFIX|]product_variation_options WHERE find_in_set(voptionid, pvc.vcoptionids))
							ELSE
								''
						END AS vovalues
					...
					LEFT JOIN [|PREFIX|]product_variation_options pvo ON pvo.voptionid = pvc.vcoptionids
					 " . $vendorSql . "
					ORDER BY ". $sortField . " " . $sortOrder . ", productid, vcsku"
				;
			} else {
				$query = "
					SELECT productid, prodcode, prodname, prodnumviews, prodinvtrack, 0 as combinationid,
						CASE prodinvtrack
							WHEN '0' THEN
								-1
							WHEN '1' THEN
				...
				while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
					...
					if ($productHasVariations && $showVariations) {
						...
						$GLOBALS['OrderGrid'] .= '<tr class="GridRow" onmouseover="this.className=\'GridRowOver\';" onmouseout="this.className=\'GridRow\';" title="' . isc_html_escape($variationRowTitle) . '">' .
													...
													'<td class="Variation VariationItem ' . $GLOBALS['SortedFieldNameClass'] . '">' . isc_html_escape($row['vovalues']) . '</td>' .
													'<td>&nbsp;</td>' .
													'<td class="Variation VariationStock ' . $GLOBALS['SortedFieldStockClass'] . '">' . $row['vcstock'] . '</td>' .
													'<td>&nbsp;</td>' .
													'</tr>';
					}
