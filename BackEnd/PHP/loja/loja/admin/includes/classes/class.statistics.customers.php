...
class ISC_ADMIN_STATISTICS_CUSTOMERS extends ISC_ADMIN_STATISTICS
{
	...
	*/
	public function CustomerStats()
	{
		...
		$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("stats.customers");
			...
			if($total_customers > 0) {
				...
				$query = sprintf("
					SELECT
						...
						SUM(ordtotalamount) AS orderamount
					FROM
						[|PREFIX|]customers
						LEFT JOIN [|PREFIX|]orders ON (ordcustid = customerid AND ordstatus IN (".implode(',', GetPaidOrderStatusArray())."))
					WHERE
				...
				while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
					...
					if($row['numorders'] > 0) {
						$viewOrders = sprintf("<a href=\"index.php?ToDo=viewOrders&searchQuery=%s\" target=\"_blank\">%s</a>", urlencode($row['custconemail']), GetLang('StatsViewOrders'));
					} else {
				...
				$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("stats.customers.bydategrid");
				$GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate();
			}
		}
	}
