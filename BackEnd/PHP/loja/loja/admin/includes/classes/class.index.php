<?php
class ISC_ADMIN_INDEX
{
	/**
	 * @var object A reference of the database class to work with locally.
	 */
	private $db = null;

	/**
	 * @var object A reference of the template class to work with locally.
	 */
	private $template = null;

	/**
	 * @var object A reference of the authorization class to work with locally.
	 */
	private $auth = null;

	/**
	 * @var object A reference of the processing/engine class to work with locally.
	*/
	private $engine = null;
	...
	private function SetupVersionCheck()
	{
		...
		return $this->template->GetSnippet('DashboardVersionCheck');
	}
	...
	public function GenerateGettingStarted()
	{
		...
		return array(
			'steps' => $this->template->GetSnippet('DashboardGettingStarted'),
	...
	public function GenerateOrderBreakdownGraph()
	{
		...
		$query = "
			SELECT
				...
				SUM(ordtotalamount) AS totalrevenue, COUNT(*) AS numorders, orddate
			FROM [|PREFIX|]orders
			WHERE ordstatus IN (".implode(',', GetPaidOrderStatusArray()).") AND orddate >= '".$graphFrom."' ".$vendorAdd."
		...
		for($i=$graphFrom; $i <= time(); $i += 86400) {
			...
			$graphRows .= $this->template->GetSnippet('DashboardOrderBreakdownGraphRow');
		}
