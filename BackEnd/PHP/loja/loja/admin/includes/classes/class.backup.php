...
class ISC_ADMIN_BACKUP
{
	...
	public function _UpdateProgress($message = '')
	{
		...
		if(count($this->_ImageItems) != 0) {
			$total += count($this->_ImageItems);
			$done += $this->_ImageProgress;
		}

		if(count($this->_ProductItems) != 0) {
	...
	public function _GetBackupList()
	{
		...
		if(!is_dir(ISC_BACKUP_DIRECTORY)) {
			@mkdir(ISC_BACKUP_DIRECTORY, ISC_WRITEABLE_DIR_PERM);
		}
	...
	private function getProductImagesList()
	{
		...
		while (($subpath = $dh->read()) !== false) {
			if ($subpath{0} == "." || !is_dir($path . "/" . $subpath)) {
				continue;
			}

			$subdh = @dir($path . "/" . $subpath);

			if (!$subdh) {
				continue;
			}

			while (($file = $subdh->read()) !== false) {
				if (!is_file($path . "/" . $subpath . "/" . $file)) {
					continue;
				}

				// Just return the subpath as that is the only thing we'll be using
				$images[] = $subpath . "/" . $file;
			}
...
class ISC_ADMIN_BACKUP_LOCAL
{
	...
	{
		return mkdir($this->pwd . "/" . $directory, ISC_WRITEABLE_DIR_PERM);
	}
