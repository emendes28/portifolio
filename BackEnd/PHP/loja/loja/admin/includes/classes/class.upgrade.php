 ...
 * ISC_ADMIN_UPGRADE
 *
 * This class handles the upgrade wizard used for upgrading Loja Virtual V2010.
 *
 * @author Chris Boulton
 * @copyright 	Copyright (c) 2004-2008 www.google.com.br.
 * @package 	Loja Virtual V2010
 */
...
class ISC_ADMIN_UPGRADE
{
	...
	private function ShowUpgradeErrors()
	{
		...
		$errorReport = "Relatorio de atualizacao\n";
		$errorReport .= "----------------------------------------------\n";
		$errorReport .= gmdate("r")."\n";
		$errorReport .= "\n";
		$errorReport .= "URL: ".$GLOBALS['ShopPath']."\n";
		$errorReport .= "Email: ".GetConfig('AdminEmail')."\n";
		$errorReport .= "Edicao: ".$GLOBALS['ProductEdition']."\n";
		$errorReport .= "\n";

		$errorReport .= "Detalhes:\n";
		$errorReport .= "----------------\n";
		$errorReport .= "De: ".$GLOBALS['FromVersion']." (".$dbVersion.")\n";
		$errorReport .= "Para: ".PRODUCT_VERSION." (".PRODUCT_VERSION_CODE.")\n";
		$errorReport .= "\n";
		...
		foreach($this->upgradeSession['upgradePath'] as $module => $steps) {
			...
			foreach($steps as $step) {
				...
				if($module == $this->upgradeSession['currentStep']['module'] && $step == $this->upgradeSession['currentStep']['step']) {
					$errorReport .= " <-- Falhou aqui";
				}
				$errorReport .= "\n";
			}
		}
		$errorReport .= "\n";
		$errorReport .= "Servidor:\n";
		$errorReport .= "---------------------\n";
		$errorReport .= "PHP Versao: ".phpversion()."\n";
		$errorReport .= "MySQL Versao: ".mysql_get_server_info()."\n";
