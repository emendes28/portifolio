<?php
	...
	class ISC_ADMIN_INSTALL
	{
		...
		public $FoldersToCheck = array("addons", "admin/backups", "cache", "cache/feeds", "cache/tplthumbs", "cache/logos", "cache/import", "product_downloads", "product_images", "templates", "config/config.php", "config", "language/en/front_language.ini", "product_downloads/a", "product_downloads/b", "product_downloads/c", "product_downloads/d", "product_downloads/e", "product_downloads/f", "product_downloads/g", "product_downloads/h", "product_downloads/i", "product_downloads/j", "product_downloads/k", "product_downloads/l", "product_downloads/m", "product_downloads/n", "product_downloads/o", "product_downloads/p", "product_downloads/q", "product_downloads/r", "product_downloads/s", "product_downloads/t", "product_downloads/u", "product_downloads/v", "product_downloads/w", "product_downloads/x", "product_downloads/y", "product_downloads/z", "product_downloads/import", "product_images/a", "product_images/b", "product_images/c", "product_images/d", "product_images/e", "product_images/f", "product_images/g", "product_images/h", "product_images/i", "product_images/j", "product_images/k", "product_images/l", "product_images/m", "product_images/n", "product_images/o", "product_images/p", "product_images/q", "product_images/r", "product_images/s", "product_images/t", "product_images/u", "product_images/v", "product_images/w", "product_images/x", "product_images/y", "product_images/z", "product_images/uploaded_images", "product_images/header_images", "cache/datastore", 'cache/spool', 'product_images/wrap_images', 'product_images/configured_products', 'product_images/configured_products_tmp');
		...
		public function __construct()
		{
			...
			if(!GetConfig("isSetup")) {
				...
				$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->LoadLangFile('install');
		...
		private function ShowInstallErrors($message, $errors, $showRetry = false, $onRunInstall = false)
		{
			if($this->apiMode == 'cli') {
				...
				// Only on the main screen, just set globals
					...
					if($showRetry == true) {
						$GLOBALS['PermissionErrors'] .= "<br />Apos Feito Atualize a Pagina.";
					}
		...
		private function SetupApiInstall()
		{
			...
			if(isset($request->install->storeCountryLocationId)) {
				$_POST['StoreCountryLocationId'] = 30;
			}
			else {
				$_POST['StoreCountryLocationId'] = 30; // United States
			}

			if(isset($request->install->storeCurrencyCode)) {
				$_POST['StoreCurrencyCode'] = 'BRL';
			}
			else {
				$_POST['StoreCurrencyCode'] = 'BRL';
			}
		...
		private function GetTrialLicenseKey()
		{
			...
			if($response === false || !is_object($xml)) {
				$this->ShowInstallErrors('', $errors, false, false);
				exit;
			}
			...
			if($xml->status == "OK") {
				...
				$this->ShowInstallErrors(''.$xml->error.')', $errors, false, false);
				exit;
			}
		}
		...
		private function RunInstall()
		{
			...
			if($connection) {
				...
				if($mysql_check < 0) {
					...
					$GLOBALS["ISC_CLASS_TEMPLATE"]->SetTemplate("install.schema");
					$queries = $GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate(true);
					$queries = str_replace("\r", "\n", str_replace("\r\n", "\n", $queries));
					$queries = explode(";\n", $queries);
					$GLOBALS["ISC_CLASS_DB"]->Query("start transaction");

					require_once(dirname(__FILE__) . "/class.user.php");
					$user = GetClass('ISC_ADMIN_USER');
					$pass = $_POST['UserPass'];
					$token = $user->_GenerateUserToken();
					...
					foreach($queries as $query) {
						...
						$query = str_replace("%%PASS%%", md5($pass), $query);
					...
					$query = $GLOBALS['ISC_CLASS_DB']->Query("SELECT pk_userid FROM [|PREFIX|]users WHERE username='admin'");
					$user_id = $GLOBALS['ISC_CLASS_DB']->Fetch($query);
					$user_id = $user_id["pk_userid"];
					...
					if(file_exists(ISC_BASE_PATH.'/custom/install.schema.tpl')) {
						$oldTemplateDir = ISC_BASE_PATH.'/admin/templates';
						$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplateBase(ISC_BASE_PATH.'/custom');
						$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate('install.schema');
						$queries = $GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate(true);
						$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplateBase($oldTemplateDir);
						$queries = str_replace("\r", "\n", str_replace("\r\n", "\n", $queries));
						$queries = explode(";\n", $queries);
					...
					if($GLOBALS["ISC_CLASS_DB"]->Error() == "") {
						...
						$GLOBALS['ISC_NEW_CFG']['HostingProvider'] = "WWW.cliquemania.com";
						...
						if ($settings->CommitSettings()) {
							...
							if($this->apiMode == 'cli') {
								...
								$GLOBALS["ISC_CLASS_TEMPLATE"]->SetTemplate("install.done");
			...
			foreach($folders as $folder) {
				switch($folder[1]) {
					case NOT_WRITABLE: {
						...
						$message = "The ".$type." <strong>" . $folder[0] . "</strong> SEM PERMICAO DE ESCRITA. DE PERMICAO NA MESMA ";
						if (isset($folder[2]) && $folder[2] === true) {
							$message .= "646 OU 664 OU 666";
						} else {
							$message .= "757 OU 775 OU 777";
						}
						$code = "filePermissions";
						break;
					}
					case DOESNT_EXIST: {
						$message = "O ARQUIVO/PASTA <strong>" . $folder[0] . "</strong> NAO EXISTE. CRIE A MESMA.";
						$code = "doesntExist";
						break;
					}
					default: {
		...
		private function GetVariables($Error = false, $Message = "")
		{
			...
			if(isset($GLOBALS['LicenseKey']) && isset($GLOBALS['LE'])) {
				...
				$GLOBALS["ISC_CLASS_ADMIN_AUTH"]->SavePerms("");
				if(isset($GLOBALS['KM'])) {
					$GLOBALS['Message'] = "<h3 style='padding-bottom:10px; color:red'>" . GetLang("Oops") . "</h3>" . $Message;
				}
			}
			...
			$GLOBALS["ISC_CLASS_TEMPLATE"]->SetTemplate("install.form");
			echo $GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate();
		}
		...
		{
			$GLOBALS["ISC_CLASS_TEMPLATE"]->SetTemplate("install.sample");
			$queries = $GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate(true);
