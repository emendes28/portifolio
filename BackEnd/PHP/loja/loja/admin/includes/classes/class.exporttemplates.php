...
* @author Ray Ward
*/
class ISC_ADMIN_EXPORTTEMPLATES
{
	/**
	...
	*/
	private function BuildTemplatesGrid()
	{
		...
		while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
			...
			$GLOBALS["ISC_CLASS_TEMPLATE"]->SetTemplate("exporttemplates.manage.grid.row");
		...
		$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("exporttemplates.manage.grid");
	...
	*/
	private function CreateTemplate($loadFromPost = false)
	{
		...
		$GLOBALS["ISC_CLASS_TEMPLATE"]->SetTemplate("exporttemplates.form");
		$GLOBALS["ISC_CLASS_TEMPLATE"]->ParseTemplate();
	}
	...
	*/
	private function GetFieldGrid($fieldType, $fields, $use_defaults = false, $setType = true, $depth = 0)
	{
		...
		foreach ($fields as $id => $field) {
			...
			// render the row
			$GLOBALS["ISC_CLASS_TEMPLATE"]->SetTemplate("exporttemplates.form.grid.row");
			$gridData .= "\n" . $GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate(true);
		}
