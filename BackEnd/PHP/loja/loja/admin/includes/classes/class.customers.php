	...
	class ISC_ADMIN_CUSTOMERS
	{
			...
			if ($numCustomers > 0) {
				while ($row = $GLOBALS["ISC_CLASS_DB"]->Fetch($customerResult)) {
					...
					if ($GLOBALS["ISC_CLASS_ADMIN_AUTH"]->HasPermission(AUTH_Edit_Customers)) {
						...
						$GLOBALS['StoreCredit'] = $GLOBALS["ISC_CLASS_TEMPLATE"]->GetSnippet('CustomerRowStoreCredit');
						...
						$GLOBALS['ViewNotesLink'] = "<a href='#' class='" . $HasNotesClass . " ViewNotesLink' onclick='Customers.ViewNotes(%%GLOBAL_CustomerId%%); return false;'>" . GetLang("CustomerNotesLink") . "</a>";
		...
		public function _GetCustomerList($Start, $SortField, $SortOrder, &$NumCustomers, $AddLimit=true)
		{
			...
			if($GLOBALS['ISC_CLASS_ADMIN_AUTH']->GetVendorId()) {
				$query .= " AND ordvendorid = " . $GLOBALS['ISC_CLASS_ADMIN_AUTH']->GetVendorId();
			}
		...
		*/
		...
		public function ManageGroupDiscountGrid($data=null)
		{
			...
			foreach ($items as $item) {
				...
				$html .= $GLOBALS["ISC_CLASS_TEMPLATE"]->GetSnippet('CustomerGroup' . ucfirst(isc_strtolower($type)) . 'Row');
			}
			...
			$GLOBALS["ISC_CLASS_TEMPLATE"]->SetTemplate("customers.groups.discount.grid");
			return $GLOBALS["ISC_CLASS_TEMPLATE"]->ParseTemplate(true);
		}
		...
		private function buildHiddenDiscountFields($discounts, &$products, &$categories)
		{
			...
			foreach ($discounts as $discount) {
				...
				$html .= $GLOBALS["ISC_CLASS_TEMPLATE"]->GetSnippet('CustomerGroupHiddenBlock');
			}
