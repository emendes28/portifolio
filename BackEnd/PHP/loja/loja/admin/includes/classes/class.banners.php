<?php
	...
	class ISC_ADMIN_BANNERS
	{
		...
		private function _GetBannerList($Query, $SortField, $SortOrder, &$NumResults)
		{
			...
			if($Query != '') {
				$queryWhere .= " AND LOWER(name) LIKE '%".$Query."%'";
			}
		...
		// modificacao
            ...
            switch ($banner['location']) {
               ...
               case "direito": {
               $GLOBALS['Location'] .= sprintf(" (%s)", 'Lado Direito');
               break;
            }
			
			 case "esquerdo": {
               $GLOBALS['Location'] .= sprintf(" (%s)", 'Lado Esquerdo');
               break;
            }
               case "bottom": {
		...
		private function ManageBanners()
		{
			...
			$GLOBALS["ISC_CLASS_TEMPLATE"]->SetTemplate("banners.manage");
		...
		private function AddBannerStep1()
		{
			...
			$GLOBALS["ISC_CLASS_TEMPLATE"]->SetTemplate("banner.form");
			$GLOBALS["ISC_CLASS_TEMPLATE"]->ParseTemplate();
		}
		...
		*/
		private function _CommitBanner($ExistingBannerId = 0)
		{
			...
			if(isset($_POST['wysiwyg_html'])) {
				...
				$phrase = $_POST['wysiwyg'];
				$tira = array("<p>", "</p>");
				$poe = array("","");
				$content = str_replace($tira, $poe, $phrase);
			}
		...
		private function EditBannerStep1()
		{
			if (isset($_GET['bannerId']) && is_numeric($_GET['bannerId'])) {
				...
				if ($banner = $this->GetBannerById($banner_id)) {
					...
					$GLOBALS['Texto'] = $banner['content'];
					$GLOBALS['FLASH'] = urldecode($banner['content']);
					...
					if ($banner['location'] == "top") {
						...
						$GLOBALS['MostraFlash'] = "$('#fresh').css('display', '');";
					} else if ($banner['location'] == "bottom") {
						$GLOBALS['IsLocationBottom'] = "selected=\"selected\"";
						$GLOBALS['MostraNormal'] = "$('#normal').css('display', '');";

					} else if ($banner['location'] == "direito") {
                    $GLOBALS['IsLocationDireito'] = "selected=\"selected\"";
					$GLOBALS['MostraNormal'] = "$('#normal').css('display', '');";
                    } else {
					$GLOBALS['IsLocationEsquerdo'] = "selected=\"selected\"";
					$GLOBALS['MostraNormal'] = "$('#normal').css('display', '');";
					}
