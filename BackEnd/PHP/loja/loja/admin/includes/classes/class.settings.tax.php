...
class ISC_ADMIN_SETTINGS_TAX
{
	...
	private function ManageTaxSettings($messages=array())
	{
		...
		if(empty($GLOBALS['Message']) && (isset($_GET['wizard']) && $_GET['wizard']==1) && !in_array('taxSettings', GetConfig('GettingStartedCompleted')) && !GetConfig('DisableGettingStarted')) {
			$GLOBALS['GettingStartedTitle'] = GetLang('WizardTaxSettings');
			$GLOBALS['GettingStartedContent'] = GetLang('WizardTaxSettingsDesc');
	...
	private function SaveTaxSettings()
	{
		...
		// Just configured tax
		$redirectUrl = 'index.php?ToDo=viewTaxSettings';
		...
		if ($settings->CommitSettings($messages)) {
			...
			if($GLOBALS['ISC_NEW_CFG']['TaxTypeSelected'] == 1) {
				FlashMessage(GetLang('SettingsSavedSuccessfullyNewTaxStyle'), MSG_SUCCESS, $redirectUrl);
			} else {
	...
	private function AddTaxRate()
	{
		...
		if(!empty($_GET['wizard'])) {
			$GLOBALS['FormAction'] = '&wizard=1';
		}
		$GLOBALS['Message'] = GetFlashMessageBoxes();
		$GLOBALS['SaveAndContinue'] = GetLang('SaveAndAddAnother');
	...
	private function GetTaxRateDataFromPost()
	{
		...
		if ($data['taxratebasedon'] == 'subtotal_and_shipping' && isset($_POST['taxshippingfortaxableorder'])) {
			$data['taxshippingfortaxableorder']	= 1;
		}
		else {
			$data['taxshippingfortaxableorder'] = 0;
		}
	...
	private function SaveNewTaxRate()
	{
		...
		if (!$this->TaxRateSetupFor($data['taxratecountry'], $data['taxratestates'])) {
			$newTaxRate = array(
				...
				'taxshippingfortaxableorder' => $data['taxshippingfortaxableorder']
			);
			...
			if ($GLOBALS['ISC_CLASS_DB']->Error() == "") {
				...
				$addAnotherAppend = '';
				if(!in_array('taxSettings', GetConfig('GettingStartedCompleted')) || !empty($_GET['wizard'])) {
					GetClass('ISC_ADMIN_ENGINE')->MarkGettingStartedComplete('taxSettings');
					$redirectUrl = 'index.php';
					$addAnotherAppend = '&wizard=1';
				}
				else {
					$redirectUrl = 'index.php?ToDo=viewTaxSettings&currentTab=2';
				}

				if(isset($_POST['addAnother'])) {
					$redirectUrl = 'index.php?ToDo=settingsAddTaxRate'.$addAnotherAppend;
				}

				FlashMessage(GetLang('TaxRateAddedSuccessfully'), MSG_SUCCESS, $redirectUrl);
			}
			else {
				FlashMessage(GetLang('TaxRateNotAdded'), MSG_ERROR);
				$this->AddTaxRate();
				return;
			}
		}
			...
			FlashMessage(GetLang('TaxRateAlreadySetup'), MSG_ERROR);
			$this->AddTaxRate();
			return;
		}
	}
	...
	private function EditTaxRate()
	{
		...
		$GLOBALS['CancelMessage'] = GetLang('CancelAddTaxRate');
		$GLOBALS['Message'] = GetFlashMessageBoxes();
		$GLOBALS['SaveAndContinue'] = GetLang('SaveAndContinueEditing');
		...
		if (isset($_GET['taxRateId'])) {
			...
			if ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
				...
				if ($row['taxshippingfortaxableorder']) {
					$GLOBALS['TaxShippingForTaxableOrder'] = 'checked="checked"';
				}
	...
	private function SaveUpdatedTaxRate()
	{
		...
		if ($GLOBALS['ISC_CLASS_DB']->Error() == "") {
			$redirectUrl = 'index.php?ToDo=viewTaxSettings';
			if(isset($_POST['addAnother'])) {
				$redirectUrl = 'index.php?ToDo=settingsEditTaxRate&taxRateId='.(int)$data['taxrateid'];
			}

			FlashMessage(GetLang('TaxRateUpdatedSuccessfully'), MSG_SUCCESS, $redirectUrl);

			// Log this action
			$GLOBALS['ISC_CLASS_LOG']->LogAdminAction($data['taxrateid'], $data['taxratename']);
		}
		else {
			FlashMessage(sprintf(GetLang('TaxRateNotUpdated'), $GLOBALS['ISC_CLASS_DB']->Error()), MSG_ERROR);
			$this->EditTaxRate();
			return;
		}
	}
