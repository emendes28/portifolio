...
" ;
			...
			for ( $c = 0; $c < count( $users ); ++$c )
			{
				...
				if ( $user['login'] )
				{
					$query = "UPDATE chat_asp SET trans_message = 'Se gostaria de receber uma cópia deste bate-papo, por favor  insira seu email abaixo e clique em enviar.', trans_email = '$trans_email' WHERE aspID = $user[aspID]" ;
					database_mysql_query( $dbh, $query ) ;
				}
			}
...
<tr>
	<td valign="top"><span class="basetxt">
		...
		<ol>
			<form method="GET" action="<?php echo $PATCH_VERSION ?>_patch.php" method="POST" name="form">
