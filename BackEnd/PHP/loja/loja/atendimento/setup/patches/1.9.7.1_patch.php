<?
	...
	$PATCH_VERSION = "1.9.7.1" ;
	$previous_version = "1.9.7" ;
	$success = 0 ;
	$action = $error = "" ;
	...
	/*
	// because on v1.9.7.2, the version on setup was set to v1.9.7, there
	// is an error on versions and DB.  thus, we need to check table to
	// see if it is v1.9.7.2 or v1.9.7
	*/
	$query = "SELECT ordernum FROM chatuserdeptlist LIMIT 1" ;
	database_mysql_query( $dbh, $query ) ;
	if ( $dbh['error'] == "None" )
	{
		// create the actual patch file
		$date = date( "D m/d/y h:i a", time() ) ;
		$success_string = "DO NOT DELETE OR SYSTEM MAY PATCH AGAIN!  THAT'S NOT GOOD!\n[$date] PATCH SUCCESSFUL from v$1.9.7 to v$1.9.7.2 (previous version had wrong version during install - bug fixed)\n" ;
		...
		$version_string = "0LEFT_ARROW0? \$PHPLIVE_VERSION = \"1.9.7.2\" ; ?0RIGHT_ARROW0" ;
