...
class ISC_ENTITY_BASE
{
	...
	private function parseInput($input)
	{
		...
		foreach ($input as $column => $value) {
			...
			switch (isc_strtolower($this->schema[$column])) {
				...
				case "price":
					// values for prices are handled external to the entity
					// so just leave $value the same
	 ...
	 *
	 * Method will return TRUE/FLSE depending if the record exists
	 *
	 * @access public
	 * @param mixed $nodeId The database record ID OR numeric array of IDs if multiple primary keys OR an associative
	 *                      array of fields to match up againt (fields must be in the record schema)
	 ...
	 * @access public
	 * @param array $input The data array
	 * @param mixed $nodeId The optional database record ID OR numeric array of IDs if multiple primary keys OR
	 *                      an associative array of fields to match up againt (fields must be in the record schema).
	 *                      Default is $input[$this->primaryKeyName]
	 ...
	 * @access public
	 * @param mixed $nodeId The database record ID OR numeric array of IDs if multiple primary keys OR an associative
	 *                      array of fields to match up againt (fields must be in the record schema). Default is
	 *                      $input[$this->primaryKeyName]
	 * @return bool TRUE if the record was successfully updated, FALSE on error
	 */
