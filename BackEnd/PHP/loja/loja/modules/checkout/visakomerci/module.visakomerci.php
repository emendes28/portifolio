<?php
class CHECKOUT_VISAKOMERCI extends ISC_CHECKOUT_PROVIDER
{
	...
	{
		parent::__construct();
		$this->_name = "Cartão Visa - Redecard";
		$this->_image = "images.jpeg";
		$this->_description = "Modulo de Pagamento Online Visa Redecard de Cartões de Creditos.";
...
$msg .="<a href=\"javascript:window.open('".$GLOBALS['ShopPath']."/modules/checkout/visakomerci/pagar.php?pedido=".$order['orderid']."&acao=".$ac."&pacela=".$jj."&key=c2VtanVyb3M=','popup','width=800,height=800,scrollbars=yes');void(0);\"><font size='2'><b>".$j."x</b> de <b>".number_format($valors/$j, 2, '.', ',')."</b> s/ juros</font></a><br>";
}else{
$msg1 .="<a href=\"javascript:window.open('".$GLOBALS['ShopPath']."/modules/checkout/visakomerci/pagar.php?pedido=".$order['orderid']."&acao=06&pacela=".$j."&key=Y29tanVyb3M=','popup','width=800,height=800,scrollbars=yes');void(0);\"><font size='2'><b>".$j."x</b> de <b>".number_format($parcelas, 2, '.', ',')."</b> c/ juros</font></a><br>";
}

}

$help .= "<div class='FloatLeft'><img src='".$GLOBALS['ShopPath']."/modules/checkout/visakomerci/images/final.gif'><br><br>";
...
<a href='.$GLOBALS["ShopPath"].'/modules/checkout/visakomerci/repagar.php?pedido='.$order['orderid'].' target="_blank">'.$GLOBALS["ShopPath"].'/modules/checkout/visakomerci/repagar.php?pedido='.$order['orderid'].'</a><br>-->
</div>';
