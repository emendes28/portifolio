<?php
	...
	if ( $action == "execute" )
	{
		...
		else
		{
			...
			$query = "DROP TABLE browser_err" ;
			database_mysql_query( $dbh, $query ) ;
			$query = "ALTER TABLE chatrequests CHANGE question question TEXT NOT NULL" ;
			database_mysql_query( $dbh, $query ) ;
			$query = "ALTER TABLE chat_asp CHANGE login login VARCHAR( 25 ) NOT NULL" ;
			database_mysql_query( $dbh, $query ) ;
			$query = "ALTER TABLE chat_admin DROP company" ;
			database_mysql_query( $dbh, $query ) ;
			$query = "ALTER TABLE chat_admin ADD theme VARCHAR( 25 ) NOT NULL AFTER pic" ;
			database_mysql_query( $dbh, $query ) ;
			$query = "ALTER TABLE chatdepartments ADD show_que TINYINT( 1 ) NOT NULL AFTER transcript_expire" ;
			database_mysql_query( $dbh, $query ) ;
...
<tr>
	<td valign="top"><span class="basetxt">
		...
		<ol>
			...
			<tr>
				<td><font color="#FF0000">*</font> <i>If you do not run the patch, your  system may not function normally.</i></td>
			</tr>
			</table>
		</ol>
		<?php endif ?>
