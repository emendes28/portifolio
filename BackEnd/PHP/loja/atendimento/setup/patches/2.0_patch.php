...
<?
	...
	if ( $action == "execute" )
	{
		...
		else
		{
			...
			{
				$user = $users[$c] ;
				include( "../../web/$user[login]/$user[login]-conf-init.php" ) ;
			...
			$query = "CREATE TABLE chatclicktracking (
			...
			database_mysql_query( $dbh, $query ) ;
			$query = "ALTER TABLE chatrefer ADD `trackID` INT(10) UNSIGNED NOT NULL AFTER `created`" ;
			database_mysql_query( $dbh, $query ) ;
			$query = "ALTER TABLE chatrefer ADD INDEX (`trackID`)" ;
			database_mysql_query( $dbh, $query ) ;
			$query = "ALTER TABLE chatdepartments ADD `status_image_away` VARCHAR(20) NOT NULL AFTER `status_image_online`" ;
			database_mysql_query( $dbh, $query ) ;
			$query = "ALTER TABLE chatdepartments ADD `away_message` MEDIUMTEXT NOT NULL AFTER `message`" ;
			database_mysql_query( $dbh, $query ) ;
			$query = "ALTER TABLE chat_admin ADD `pic` VARCHAR(25) NOT NULL" ;
			database_mysql_query( $dbh, $query ) ;
