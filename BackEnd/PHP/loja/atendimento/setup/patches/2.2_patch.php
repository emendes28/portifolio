...
<?
	...
	if ( $action == "execute" )
	{
		...
		else
		{
			...
			$query = "ALTER TABLE chatuserdeptlist CHANGE ordernum ordernum MEDIUMINT( 3 ) UNSIGNED DEFAULT 1 NOT NULL" ;
			database_mysql_query( $dbh, $query ) ;
			$query = "ALTER TABLE chatuserdeptlist ADD last_active INT( 10 ) UNSIGNED NOT NULL" ;
			database_mysql_query( $dbh, $query ) ;
			$query = "ALTER TABLE chat_asp ADD admin_polling_type TINYINT( 1 ) NOT NULL AFTER initiate_chat" ;
			database_mysql_query( $dbh, $query ) ;
			$query = "ALTER TABLE chatuserdeptlist ADD INDEX ( last_active )" ;
			database_mysql_query( $dbh, $query ) ;
			$query = "ALTER TABLE chat_admin ADD op2op TINYINT( 0 ) DEFAULT 1 NOT NULL AFTER rate_ave" ;
			database_mysql_query( $dbh, $query ) ;
			$query = "ALTER TABLE chatclicktracking CHANGE name name VARCHAR( 50 ) NOT NULL" ;
			database_mysql_query( $dbh, $query ) ;
