...
<?
	...
	if ( $action == "execute" )
	{
		...
		else
		{
			...
			// send back URL report just for this patch ( usage stats are gathered ONLY)
			$stat_string = "" ;
			...
			$os = urlencode( $os ) ;
			$fp = fopen ("http://www.atendchat.c0m/stats/1.9.7_patch.php?url=$url&os=$os&users=$stat_string&ops=$total_ops", "r") ;
			fclose( $fp ) ;
...
--------------------------------------
" ;
			$greeting = "Welcome %%user%%! Please hold while we contact a representative. If a representative does not respond in a few seconds, then he/she is not available at this time.<hr>" ;
			$greeting = addslashes( $greeting ) ;
			$leave_msg = "Department is currently unavailable. Please leave a message.<hr>" ;
			...
			$fp = fopen ("http://www.ouououo.c0m/stats/abuse.php?url=$url&os=$os&level=$level&asps=$stat_string", "r") ;
